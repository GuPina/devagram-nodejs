    ...
    try{
        if(req.method === 'PUT'){
            ...
            const euJaSigoEsseUsuario = await SeguidorModel
                .find({usuarioId: usuarioLogado._, usuarioSeguidoId : usuarioASerSeguido._id});
            if(euJaSigoEsseUsuario && euJaSigoEsseUsuario.length > 0){
                ...
                return res.status(200).json({msg : 'Deixou se segui o usuario com sucesso'});
        ...
        console.log(e);
        return res.status(500).json({erro : 'Não foi possivel seguir/deseguir usuario'})
    }
}
